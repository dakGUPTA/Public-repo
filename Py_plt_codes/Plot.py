...
#d8_format = int(input('Enter one for d8 format else zero : '))
d8_format = 1 
...
while step>-1 and step<(numsteps+1):
    ...
    print(step)
    if (d8_format == 1):
        data = np.loadtxt(f"{filedir0}/prof_gp.{step:07d}")
    else:
    ...
    plt.savefig(f'{filedir1}/phi_{step}.png',dpi=300)
    ...
    plt.savefig(f'{filedir1}/comp_{step}.png', dpi=300)
